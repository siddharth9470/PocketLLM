...
export default function ChatScreen({ navigation, route }: ChatsStackScreenProps<'Chat'>) {
  ...
  const downloadedModelIds = useDownloadStore((state) => state.getDownloadedModelIds());

  const [draft, setDraft] = useState('');
  const listRef = useRef<FlatList>(null);

  const selectedModelId = conversation?.modelId ?? '';
    ...
    navigation.setOptions({
      headerRight: () => (
        <ModelPicker
          ...
          availableModelIds={
            downloadedModelIds.length > 0 ? downloadedModelIds : [selectedModelId]
          }
          onSelect={(modelId) => setConversationModel(conversationId, modelId)}
        />
      ),
    });
  }, [
